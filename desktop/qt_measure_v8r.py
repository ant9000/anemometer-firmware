...
class ApplicationWindow(QtWidgets.QMainWindow):
        ...
        for i, axis in enumerate(self.get_axes()):
            for item in measure.get(axis, []):
                if "ch101" in item:
                    ...
                    self.CALIBRATION_INPUT[f"phi_{axis}{sensor}"].append(phi[SAMPLE])
    ...
    def compute_calibration(self):
        ...
        for axis in self.get_axes():
            for sensor in [0,1]:
                phi = self.CALIBRATION_INPUT[f"phi_{axis}{sensor}"]
                phi_array = np.array(self.CALIBRATION_INPUT[f"phi_{axis}{sensor}"])
    ...
    def parse_measure(self, measure):
        ...
        for i, axis in enumerate(self.get_axes()):
            ...
            if f"{axis}0" in self.TOF and f"{axis}1" in self.TOF:
                v_air[axis] = 0.5 * (DIST / self.TOF[f"{axis}0"] - DIST / self.TOF[f"{axis}1"])
                v_suono[axis] = 0.5 * (DIST / self.TOF[f"{axis}0"] + DIST / self.TOF[f"{axis}1"])
