...
STD_SAMPLES = 30  # numero di campioni deviazione standard di default
std_buffers = {}  # dizionario per i campione della std

def update_std_samples(val):
    global STD_SAMPLES
    try:
        STD_SAMPLES = int(val)
        for buf in std_buffers.values():
            while len(buf['phase']) > STD_SAMPLES:
                buf['phase'].popleft()
            while len(buf['amp']) > STD_SAMPLES:
                buf['amp'].popleft()
    except Exception:
        pass
...
class BasePlot:
    def __init__(self):
        ...
        self.gs = self.fig.add_gridspec(1, 1, left=0.05, right=0.78, top=0.95, bottom=0.05)
        self.anim = FuncAnimation(self.fig, self.update, init_func=self.init, interval=50)
        axbox = self.fig.add_axes([0.90, 0.92, 0.15, 0.04])
        self.text_box = TextBox(axbox, 'N campioni std', initial=str(STD_SAMPLES))
        self.text_box.on_submit(update_std_samples)
        self.std_texts = {}
    def init(self):
        pass
    def update(self, n):
...
class PeakPhasesPlot(BasePlot):
    ...
    def init(self):
        self.lines = {}
        self.ax = self.fig.subplots(len(AXES), gridspec_kw={'left':0.05, 'right':0.78, 'top':0.95, 'bottom':0.05})
        if len(AXES) == 1:
            self.ax = [self.ax]
        self.ax2 = self.ax.copy()
        ...
        for i, axis in enumerate(AXES):
            for j in range(2):
                key = f"{i},{j}"
                std_buffers[key] = {'phase': collections.deque(maxlen=STD_SAMPLES), 'amp': collections.deque(maxlen=STD_SAMPLES)}
                phase_label = f"STD Fase{j}{axis}"
                amp_label = f"STD Amp{j}{axis}"
                std_buffers[key]['phase_label'] = phase_label
                std_buffers[key]['amp_label'] = amp_label
            ax_std = self.fig.add_axes([0.85, 0.65-(i*0.32), 0.20, 0.28])
            ax_std.axis('off')
            self.std_texts[i] = []
            for j in range(2):
                phase_color = self.COLORS[j % len(self.COLORS)]
                amp_color = self.COLORS2[j % len(self.COLORS2)]
                self.std_texts[i].append(ax_std.text(0, 0.75-0.15*j, '', fontsize=12, va='center', color=phase_color))
                self.std_texts[i].append(ax_std.text(0, 0.40-0.15*j, '', fontsize=12, va='center', color=amp_color))
            ...
            self.ax2[i].set_ylabel(f'Rho {axis}')
            self.ax2[i].set_ylim(0, 30000)
            for j in range(2):
                self.lines[f"{i},{j}.phase_dots"], = self.ax[i].plot(
                    x, y, 'o', color=f'{self.COLORS[j]}', markersize=1, label=f'phase {j}'
                )
                self.lines[f"{i},{j}.peak_dots"], = self.ax2[i].plot(
                    x, y, 'o', color=f'{self.COLORS2[j]}', markersize=1, label=f'rho {j}'
                )
    ...
    def update(self, n):
        ...
        for i, axis in enumerate(AXES):
            for j in [0, 1]:
                k = f"{i},{j}"
                t[k], x[k] = self.lines[f"{k}.phase_dots"].get_data(True)
                _, y[k] = self.lines[f"{i},{j}.peak_dots"].get_data(True)
        for measure in data:
            for i, axis in enumerate(AXES):
                for item in measure[axis]:
                    j, mode, rho, peak, phi, phi_sel, rho_sel = item
                    k = f"{i},{j}"
                    std_buffers[k]['phase'].append(phi_sel)
                    std_buffers[k]['amp'].append(rho_sel)
                    t[k] = np.hstack((t[k], measure["timestamp"]))
                    x[k] = np.hstack((x[k], phi_sel))
                    y[k] = np.hstack((y[k], rho_sel))
        for i, axis in enumerate(AXES):
            for j in [0, 1]:
                k = f"{i},{j}"
                ...
                phase_std = np.nanstd(list(std_buffers[k]['phase'])) if std_buffers[k]['phase'] else np.nan
                amp_std = np.nanstd(list(std_buffers[k]['amp'])) if std_buffers[k]['amp'] else np.nan
                self.std_texts[i][j*2].set_text(
                    f"{std_buffers[k]['phase_label']}: {phase_std:.2f}"
                )
                self.std_texts[i][j*2+1].set_text(
                    f"{std_buffers[k]['amp_label']}: {amp_std:.2f}"
                )
