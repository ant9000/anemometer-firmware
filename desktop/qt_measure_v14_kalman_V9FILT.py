...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def parse_measure(self, measure):
        ...
        if len(v_air) == len(self.get_axes()):
            ...
            for i, axis in enumerate(self.get_axes()):
                ...
                v_air[f'{axis}_nofilt'] = v_air_nofilt.get(axis, np.nan)  # Assicura che la chiave sia presente
                v_air[f'{axis}_kalman_only'] = v_air_kalman_only.get(axis, np.nan)
                v_air[f'{axis}_nofilt'] = v_air_nofilt.get(axis, np.nan)  # Assicura che la chiave sia presente
    ...
    def init_canvas(self):
        ...
        self.lines = {}
        # Primo grafico: Solo Kalman
        self.ax_kalman_only = self.fig.add_subplot(211)
        # Secondo grafico: Grezza
        self.ax_raw = self.fig.add_subplot(212, sharex=self.ax_kalman_only)
        ...
        for i, axis in enumerate(self.get_axes()):
            self.lines[f"{axis}.kalman_only"], = self.ax_kalman_only.plot(t, x, '-', linewidth=1.2, label=f"v_{axis} Filtro Kalman + Ampiezza", color=colors_kalman_only[axis])
