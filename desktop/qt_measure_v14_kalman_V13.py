...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.q_input.setValue(0.005)  # Default Kalman Q
    ...
    def parse_measure(self, measure):
        ...
        for i, axis in enumerate(self.get_axes()):
            ...
            for item in measure.get(axis, []):
                if "ch101" in item:
                    ...
                    delta_fase_nofilt = phi[sample0] - phi0
                    ...
                    if abs(delta_fase_nofilt) > np.pi:
                        delta_fase_nofilt = -1 * np.sign(delta_fase_nofilt) * (2 * np.pi - abs(delta_fase_nofilt))
                    # Calcolo TOF senza filtri
                    tof_nofilt = tof0 - (1000000 * delta_fase_nofilt) / (2 * np.pi * f)
                    self.TOF[f"{axis}{sensor}nofilt"] = tof_nofilt
            ...
            if f"{axis}0nofilt" in self.TOF and f"{axis}1nofilt" in self.TOF:
                dist0_0 = self.CALIBRATION[f"dist0_{axis}0"]
                dist0_1 = self.CALIBRATION[f"dist0_{axis}1"]
                v_air_nofilt[axis] = 0.5 * (dist0_0 / self.TOF[f"{axis}0nofilt"] - dist0_1 / self.TOF[f"{axis}1nofilt"])
            ...
            v_air_kalman_only[axis] = self.v_air_kalman_only_filter[axis].update(v_air_filtered[axis], Q)
        ...
        if len(v_air) == len(self.get_axes()):
            ...
            for i, axis in enumerate(self.get_axes()):
                ...
                v_air[f'{axis}_nofilt'] = v_air_nofilt.get(axis, np.nan)
    ...
    def init_canvas(self):
        ...
        self.lines = {}
        # Primo grafico: Kalman + Ampiezza
        self.ax_kalman_only = self.fig.add_subplot(311)
        # Secondo grafico: Solo Ampiezza
        self.ax_amp = self.fig.add_subplot(312, sharex=self.ax_kalman_only)
        # Terzo grafico: Grezza
        self.ax_raw = self.fig.add_subplot(313, sharex=self.ax_kalman_only)
        ...
        for i, axis in enumerate(self.get_axes()):
            self.lines[f"{axis}.kalman_only"], = self.ax_kalman_only.plot(t, x, '-', linewidth=1.2, label=f"v_{axis} Filtro Kalman + Filtro Ampiezza", color=colors_kalman_only[axis])
            self.lines[f"{axis}.amp"], = self.ax_amp.plot(t, x, '-', linewidth=1.2, label=f"v_{axis} Filtro Ampiezza", color=colors_amp[axis])
            self.lines[f"{axis}.raw"], = self.ax_raw.plot(t, x, '-', linewidth=1.2, label=f"v_{axis} Senza Filtri", color=colors[axis])
    ...
    def update_canvas(self):
        ...
        for i, axis in enumerate(self.get_axes()):
            ...
            for v_air in data:
                t_kalman_only = np.hstack((t_kalman_only, datetime.fromtimestamp(v_air["timestamp"])))
                ...
                t_raw = np.hstack((t_raw, datetime.fromtimestamp(v_air["timestamp"])))
                y_raw = np.hstack((y_raw, v_air.get(f'{axis}_nofilt', np.nan)))
            t_min = datetime.fromtimestamp(t_kalman_only[-1].timestamp() - self.duration.value())
            mask_kalman = t_kalman_only >= t_min
            mask_amp = t_amp >= t_min
            mask_raw = t_raw >= t_min
            t_kalman_only = t_kalman_only[mask_kalman]
            y_kalman_only = y_kalman_only[mask_kalman]
            t_amp = t_amp[mask_amp]
            y_amp = y_amp[mask_amp]
            t_raw = t_raw[mask_raw]
            y_raw = y_raw[mask_raw]
            y_lim_kalman[0].append(np.nanmin(y_kalman_only))
            y_lim_kalman[1].append(np.nanmax(y_kalman_only))
            ...
            self.lines[f"{axis}.kalman_only"].set_data(t_kalman_only, y_kalman_only)
            self.lines[f"{axis}.amp"].set_data(t_amp, y_amp)
            self.lines[f"{axis}.raw"].set_data(t_raw, y_raw)
        self.ax_kalman_only.set_xlim(t_min, t_kalman_only[-1])
        self.ax_amp.set_xlim(t_min, t_amp[-1])
        self.ax_raw.set_xlim(t_min, t_raw[-1])
        ...
        self.ax_kalman_only.set_title("Filtro Kalman + Ampiezza")
        self.ax_amp.set_title("Filtro Ampiezza")
        self.ax_raw.set_title("Senza Filtri")
        self.fig.canvas.draw_idle()
