...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def clear(self):
        ...
        self.phi_history = {axis: {"0": [np.nan] * 2, "1": [np.nan] * 2} for axis in "xyz"}
    ...
    def compute_calibration(self):
        ...
        for axis in self.get_axes():
            for sensor in [0,1]:
                ...
                lower_bound = np.percentile(phi_array, 25)
                upper_bound = np.percentile(phi_array, 75)
                # Filtra gli outlier
                ...
                if "ch101" in item:
                    ...
                    if not np.any(np.isnan(self.phi_history[axis][f"{sensor}"])):
                        if abs(delta_fase) <= np.pi*0.6:
                            delta_fase=(self.phi_history[axis][f"{sensor}"][-2]*(rapporto)+delta_fase*(1-rapporto))/2
                        elif abs(delta_fase) > np.pi*0.6:
                            delta_fase=self.phi_history[axis][f"{sensor}"][-2]

                    self.phi_history[axis][f"{sensor}"][-1] = delta_fase
