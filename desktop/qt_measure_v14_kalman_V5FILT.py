...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.q_input.setValue(0.001)  # Default Kalman Q
    ...
    def init_canvas(self):
        ...
        for i, axis in enumerate(self.get_axes()):
            self.lines[f"{axis}.filtered"], = self.ax.plot(t, x, '-', linewidth=1.2, label=f"v_{axis}", color=colors_filtered[axis])
    ...
    def update_canvas(self):
        ...
        y_lim = [[],[]]
        for i, axis in enumerate(self.get_axes()):
            t, y = self.lines[f"{axis}.filtered"].get_data(True)
